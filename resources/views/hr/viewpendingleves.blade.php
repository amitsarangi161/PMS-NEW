    ...
    <tbody>
      ...
      <tr style="background-color: {{$color}}">
          ...
          <td id="tid{{$viewapplie->id}}">
           <select onchange="changestatus(this.value,'{{$viewapplie->id}}','{{$viewapplie->fromdate}}','{{$viewapplie->todate}}')" class="form-control">
           <option value="">Select</option>
           <option value="ACCEPTED">ACCEPTED</option>
           <option value="REJECTED">REJECTED</option>
          </select>
        </td>
        <td><a href="/viewapplicantleave/{{$viewapplie->id}}" ><button class="btn btn-primary">VIEW</button></a></td>
      </tr>
      @endforeach
    </tbody>
...
<script type="text/javascript">
  function changestatus(value,id,fromdate,todate)
    { 
