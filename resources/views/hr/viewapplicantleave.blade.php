...
@endif
 ...
 <style type="text/css">
  ...
  <table class="table table-striped table-condensed table-bordered">
    ...
    <tfoot>
      <tr>
        <!-- <form method="post" action="/approveleave/{{$leave->id}}" enctype="multipart/form-data">
        {{csrf_field()}} -->
        <td class="text-center"><button type="submit" onclick="adjustleave('{{$leave->id}}','{{$leave->fromdate}}','{{$leave->todate}}')" class="btn btn-success pull-right">APPROVE</button></td>
      <!--  </form> -->
       <form method="post" action="/rejectleave/{{$leave->id}}" enctype="multipart/form-data">
        {{csrf_field()}}
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Adjust Leave</h4>
      </div>
      <div class="modal-body">
        <form action="/updateadjustleave" method="post">
          {{csrf_field()}}
      ...
      <tr>
         ...
         <td><input type="text" autocomplete="off" name="fromdate" id="fromdate" placeholder="Enter from date" class="form-control datepicker1" required></td> 
       </tr>
       <tr>
         <td><strong>TO DATE<span style="color: red"> *</span></strong></td>
         <td><input type="text" autocomplete="off" name="todate" id="todate" placeholder="Enter from date" class="form-control datepicker1" required></td>
...
function readURL1(input) {
  ...
  function readURL(input) {
    ...
    function adjustleave(id,fromdate,todate) {
              $("#did").val(id);
