...
<h3 class="text-center"><strong>VIEW ALL PENDING VOUCHERS</strong></h3>
       ...
       <span class="label label-warning">{{$voucher->status}}</span>
        ...
        <td>
          <form action="/cancelthisvoucher/{{$voucher->id}}"  method="post">
            {{csrf_field()}}
            {{method_field('DELETE')}}
            <button type="submit" class="btn btn-danger" onclick="return confirm('Do You want to Delete this Voucher?')">CANCEL</button>
